...
//! Safe GPIO abstraction driver in Rust.
//!
//! Demonstrates the upstream Rust driver pattern:
//! - [`GpioPin`]: safe wrapper around the C GPIO API (Layer 2)
//! - [`GpioRustModule`]: module logic with zero `unsafe` code (Layer 3)
...
extern "C" {
    ...
    fn rust_helper_misc_register(
        pin: core::ffi::c_uint,
        hw_pin: core::ffi::c_uint,
    ) -> core::ffi::c_int;
...
impl GpioPin {
    ...
    fn get(&self) -> bool {
        // SAFETY: `self.pin` is valid per type invariant.
        unsafe { rust_helper_gpio_get_value(self.pin) != 0 }
    }
}
...
impl kernel::Module for GpioRustModule {
    fn init(_module: &'static ThisModule) -> Result<Self> {
        ...
        // Register misc character device (/dev/gpio-rust) for userspace control.
        // SAFETY: `rust_helper_misc_register` is implemented in gpio_helpers.c
        // and is safe to call once; it stores the pin number and registers the
        // misc device with the kernel.
        let ret = unsafe { rust_helper_misc_register(DEFAULT_GPIO_PIN, GPIO_HW_PIN) };
        if ret != 0 {
...
impl Drop for GpioRustModule {
    fn drop(&mut self) {
        // Deregister misc device before releasing GPIO.
        // SAFETY: `rust_helper_misc_deregister` is implemented in gpio_helpers.c
        // and is safe to call once; it deregisters the misc device.
        ...
        // `self.led` is dropped here, calling `gpio_free()` automatically.
    }
}