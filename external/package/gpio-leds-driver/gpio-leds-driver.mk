# Makefile for GPIO LEDs driver package
GPIO_LEDS_DRIVER_VERSION = 1.0.0
GPIO_LEDS_DRIVER_SITE = $(BR2_EXTERNAL_NHAT092005_PATH)/../drivers/gpio-leds
GPIO_LEDS_DRIVER_SITE_METHOD = local
GPIO_LEDS_DRIVER_LICENSE = GPL-2.0
GPIO_LEDS_DRIVER_LICENSE_FILES = LICENSE

# This is a kernel module package
$(eval $(kernel-module))

# Define build commands for the kernel module
define GPIO_LEDS_DRIVER_BUILD_CMDS
	$(MAKE) -C $(LINUX_DIR) \
		M=$(@D)/src \
		$(LINUX_MAKE_FLAGS) \
		ARCH=$(KERNEL_ARCH) \
...
define GPIO_LEDS_DRIVER_INSTALL_TARGET_CMDS
	$(MAKE) -C $(LINUX_DIR) \
		M=$(@D)/src \
		$(LINUX_MAKE_FLAGS) \
		ARCH=$(KERNEL_ARCH) \
	...
	if [ -f $(@D)/build/dtbo/gpio-leds.dtbo ]; then \
...
define GPIO_LEDS_DRIVER_BUILD_DTS
	if [ -f $(@D)/dts/gpio-leds-overlay.dts ]; then \
		$(MAKE) -C $(@D)/dts \
...
GPIO_LEDS_DRIVER_POST_BUILD_HOOKS += GPIO_LEDS_DRIVER_BUILD_DTS

# Ensure kernel is built before this module
GPIO_LEDS_DRIVER_DEPENDENCIES = linux
