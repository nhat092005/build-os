# Makefile for GPIO Rust driver package
GPIO_RUST_DRIVER_VERSION = 1.0.0
GPIO_RUST_DRIVER_SITE = $(BR2_EXTERNAL_NHAT092005_PATH)/../drivers/gpio-rust
GPIO_RUST_DRIVER_SITE_METHOD = local
GPIO_RUST_DRIVER_LICENSE = GPL-2.0
...
define GPIO_RUST_DRIVER_INSTALL_TARGET_CMDS
	$(MAKE) -C $(LINUX_DIR) \
		M=$(@D)/src \
		$(LINUX_MAKE_FLAGS) \
		ARCH=$(KERNEL_ARCH) \
	...
	if [ -f $(@D)/build/dtbo/gpio-rust.dtbo ]; then \
		$(INSTALL) -D -m 0644 $(@D)/build/dtbo/gpio-rust.dtbo \
			$(BINARIES_DIR)/rpi-firmware/overlays/gpio-rust.dtbo; \
	fi
endef

# Build Device Tree overlay
define GPIO_RUST_DRIVER_BUILD_DTS
	if [ -f $(@D)/dts/gpio-rust-overlay.dts ]; then \
		$(MAKE) -C $(@D)/dts \
...
GPIO_RUST_DRIVER_POST_BUILD_HOOKS += GPIO_RUST_DRIVER_BUILD_DTS

# Ensure kernel is built before this module
GPIO_RUST_DRIVER_DEPENDENCIES = linux
